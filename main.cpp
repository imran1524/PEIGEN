...
//To simultaneously evaluate a set of S-boxes about their security-related properties
	...
	sbox3_Eva.omp_nb_threads = 1; //Number of threads = 1
...
//-v :verbose mode
//-c <value> :limitation for precomputation, e.g. -c 6 means when precomputing, all possible nodes in the graph with gate complexity no more than 6 will be expanded
//-l <value> :limitation for formal searching,e.g. -l 9, this means when searching for implementations, all possible nodes in the graph with gate complexity no more than 9 will be expanded
...
//-f <file> :define which logic library to use, e.g., -f TSMC65nm.conf will use the library specified in the file TSMC65nm.conf indicating available gates and weight of each gate
...
//-o <file> :define the name of the file which contains the name and the specification of the S-boxes to be implemented
//-z <value> : this is for resuming from Breakpoint.
...
//GENERATION
...
#if(SIZEINBIT == 3)
	t0 = clock();
	Peigen::weight::lighter<3> sbox3_GEN; //3-bit S-box
	if (argc <= 1)
	{
	...
	if (argc <= 1)
	{
        ...
        //-o <file> :define the name of the file which contains the name and the specification of the S-boxes to be implemented
        //-z <value> : this is for resuming from Breakpoint.
		args = "-v -c 6 -l 6 -p 4 -r 160 --not1 --and2 --andn2 --or2 --xor2 -f software.conf -s  .conf";
	...
	// Note, if the graph under a same configuration has already been pre computed and stored,
	// The line ``pre_compute(args)'' can be commented.
	sbox4_GEN.pre_compute(args); //for pre-computation
	sbox4_GEN.generate(args); //to generate a set of S-boxes fulfilling the given criteria.
	t1 = clock() - t0;
	cout << "Total time (mins): " << (((double)t1) / (CLOCKS_PER_SEC * 60.0)) << endl;
#endif
#endif
